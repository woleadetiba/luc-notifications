...
describe('Notification Management Update Component', () => {
  ...
  describe('ngOnInit', () => {
    it('should call status query and add missing value', () => {
      ...
      const status: IDictionary = { id: 14247 };
      notification.status = status;

      const statusCollection: IDictionary[] = [{ id: 14247 }];
      jest.spyOn(dictionaryService, 'query').mockReturnValue(of(new HttpResponse({ body: statusCollection })));
      const expectedCollection: IDictionary[] = [status, ...statusCollection];
      jest.spyOn(dictionaryService, 'addDictionaryToCollectionIfMissing').mockReturnValue(expectedCollection);

      activatedRoute.data = of({ notification });
      comp.ngOnInit();

      expect(dictionaryService.query).toHaveBeenCalled();
      expect(dictionaryService.addDictionaryToCollectionIfMissing).toHaveBeenCalledWith(statusCollection, status);
    ...
    it('should update editForm', () => {
      ...
      expect(comp.statusesCollection).toContainEqual(status);
  ...
  describe('Compare relationships', () => {
    describe('compareDictionary', () => {
      it('should forward to dictionaryService', () => {
        ...
        jest.spyOn(dictionaryService, 'compareDictionary');
        comp.compareDictionary(entity, entity2);
        expect(dictionaryService.compareDictionary).toHaveBeenCalledWith(entity, entity2);
      });
    });
  });
});
