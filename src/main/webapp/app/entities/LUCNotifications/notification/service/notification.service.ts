...
type RestOf<T extends INotification | NewNotification> = Omit<T, 'scheduledAt' | 'sentAt' | 'createdAt'> & {
...
export class NotificationService {
  ...
  protected resourceUrl = this.applicationConfigService.getEndpointFor('api/notifications', 'lucnotifications');
  protected resourceSearchUrl = this.applicationConfigService.getEndpointFor('api/notifications/_search', 'lucnotifications');
  ...
  protected convertDateFromClient<T extends INotification | NewNotification | PartialUpdateNotification>(notification: T): RestOf<T> {
    return {
      ...notification,
      scheduledAt: notification.scheduledAt?.toJSON() ?? null,
      sentAt: notification.sentAt?.toJSON() ?? null,
      createdAt: notification.createdAt?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restNotification: RestNotification): INotification {
    return {
      ...restNotification,
      scheduledAt: restNotification.scheduledAt ? dayjs(restNotification.scheduledAt) : undefined,
      sentAt: restNotification.sentAt ? dayjs(restNotification.sentAt) : undefined,
      createdAt: restNotification.createdAt ? dayjs(restNotification.createdAt) : undefined,
    };
  }
